...
public:
    ...
    {
	if(_din) _din->_fp(_din->_object);
    }
    DigitalIn_ESP8266(uint32_t gpio)
        : _gpio(gpio)
    {
	_fp = 0;
	_object = 0;
	...
	if(_fp) { gpio_set_interrupt(_gpio, _interrType, interruptHandler); }
	return E_OK;
    }
...
public:
    DigitalOut_ESP8266(uint32_t gpio)
        : _gpio(gpio)
    {
    }
    ~DigitalOut_ESP8266(){};
